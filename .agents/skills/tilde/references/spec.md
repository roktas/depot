...
- On fresh or underprovisioned hosts, run `.agents/skills/tilde/bin/bootstrap` explicitly before normal provisioning when
...
The bootstrap helper is `.agents/skills/tilde/bin/bootstrap`. It prepares the smallest baseline needed for normal
...
```bash
ssh HOST 'bash -s' < .agents/skills/tilde/bin/bootstrap
```
