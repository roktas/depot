...
`:commit [MESSAGE_HINT]` stages relevant current changes and creates a Conventional Commit using the `dev-commits`
rules. Do not push.
...
`:close [push|no-push]` performs the `dev-agents` closeout routine: check root instructions and `.agents/` consistency,
