---
name: dotagents
description: Use when creating, placing, moving, reorganizing, or updating repository-local agent assets and artifacts that follow the dotagents layout under `.agents/`, including skills, specs, tests, task state, logs, scratch files, or runtime state. Also use when deciding how `.agents/` relates to root project docs such as `PLAN.md`, `TODO.md`, `CHANGELOG.md`, `README.md`, and `AGENTS.md`.
metadata:
...
Keep root instructions short; move long reusable agent detail into a skill or spec.
...
In repositories that use the `.agents/` layout, interpret shorthand paths such as `specs/foo.md` and
`state/tasks/foo/` as `.agents/specs/foo.md` and `.agents/state/tasks/foo/` when the requested artifact is
agent-facing and the user has not explicitly asked for a top-level path.
...
After meaningful `.agents/` changes, check that specs, skills, tests, root instructions, root project docs, and local
state still agree.
